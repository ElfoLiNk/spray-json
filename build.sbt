...
OsgiKeys.importPackage := scalaVersion { sv => Seq("""scala.*;version="$<range;[==,=+);%s>"""".format(sv)) }.value
...
scalaBinaryVersion := scalaVersion(sV => if (CrossVersion.isScalaApiCompatible(sV)) CrossVersion.binaryScalaVersion(sV) else sV).value
